...
class BlockchainIngest:
    ...
    def write_next_blocks(self, start_block):
        ...
        while next_block:
            ...
            tx_number = 0
            try:
                self.__session.execute(self.__insert_block_stmt, block)
            except Exception as err:
                print("Exception ", err, " retrying...", end="\r")
                continue
            for transaction in txs:
                try:
                    self.__session.execute(self.__insert_transaction_stmt,[block_group, tx_number] + transaction)
                except Exception as err:
                    print("Exception ", err, " retrying...", end="\r")
                    continue
    ...
    while True:
        if Path(LOCKFILE).is_file():
            ...
            if last_rs:
                ...
                #os.system("cd /opt/graphsense-transformation && bash /opt/graphsense-transformation/submit.sh")
            else:
