...
mkdir data
IP=$(getent hosts btc_client | awk '{ print $1 }')
RES=$(curl --user uab:uabpassword --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "getblockhash", "params": [0] }' -H 'content-type: text/plain;' http://$IP:18332 | jq -r '.result')
...
sbt package
python3 /opt/graphsense-datafeed/continuous_ingest_mod.py -h btc_client -p 18332 -c localhost -s 300 > tx.txt
