...
if __name__ == '__main__':
	...
	while True:
	    ...
	    for element in db_list:
	    	blknum=rpc_call_blockcount(client,exchange_server,element[3])
	    	if(blknum>=str(element[7])):
		    	blkhash=rpc_call_blockhash(client,exchange_server,str(element[7]),element[3])
		    	blkinfo=rpc_call_blockinfo(client,exchange_server,blkhash,element[3])
		    	if(element[6]<=blkinfo['confirmations']):
		    		...
		    		if(element[4]=="btc"):
		    			...
		    			amount_rated=round(float(element[5])*rate_zch_btc,8)
