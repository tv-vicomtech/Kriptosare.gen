...
def hardcase(client,destiny,origen,list_tx,cant,txaddr,transid,txout,pkey,txinlist,brother,db):
    ...
    cur = db.cursor()
    query = ("SELECT * FROM destination where IP=%s")
    args=destiny[0]
    cur.execute(query,[args])
    db_add=[]
    for element in cur.fetchall():
        db_add.append(element[0])

    if(len(db_add)>=brother):
        addr=db_add[0:brother]
    else:
        brother=len(db_add)
        addr=db_add
    ...
    cur = db.cursor()
    query = ("DELETE * FROM destination where address=%s")
    cur.execute(query,[addr[0:rem]])
